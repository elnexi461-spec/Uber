...
async function apiJobsRun(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const auth = await getAuthStatus();
  if (!auth.connected) {
    sendJson(res, 403, { error: "No authenticated Uber session. Use POST /api/auth/login first." });
    return;
  }
  ...
  sendJson(res, 202, { accepted: true, pid: child.pid, startedAt, message: "Batch started. Poll GET /api/jobs for progress." });
}
